...
{
	// stores the pointer to the SDAMix instance
	typedef std::shared_ptr<class SDAMix> 	SDAMixRef;
	struct SDAMixFbo
	{
		ci::gl::FboRef					fbo;
	...
	class SDAMix {
	public:
		SDAMix(SDASettingsRef aSDASettings, SDAAnimationRef aSDAAnimation);
		static SDAMixRef					create(SDASettingsRef aSDASettings, SDAAnimationRef aSDAAnimation)
		{
			return shared_ptr<SDAMix>(new SDAMix(aSDASettings, aSDAAnimation));
		}
		void							update();
		void							updateAudio();
		void							resize();
